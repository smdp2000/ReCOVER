...
function Covid19_global() {
  ...
  useEffect(() => {
    ...
    setGlobalState({...globalState, arealist: globallist});
	}, [])
        ...
        for(i=1; i<results.data.length; i++)
        {
          if(results.data[i].length > 2){
            globallist[i-1] = results.data[i][1];
				...
				setGlobalState({...globalState, data_date: results.data[0].slice(2).map(y => y.concat('T23:00:00Z'))})
				setGlobalState({...globalState, arealist: globallist})
				setGlobalState({...globalState, case_data: results.data})
				doneLoading()
			}
		})
        ...
        for(i=1; i<results.data.length; i++)
        {
          if(results.data[i].length > 2){
            thislist[i-1] = results.data[i][1];
				...
				setGlobalState({...globalState, death_list: thislist})
				setGlobalState({...globalState, death_data: results.data})
				doneLoading()
			}
		})
        ...
        for(i=1; i<results.data.length; i++)
        {
          if(results.data[i].length > 2){
            thislist[i-1] = results.data[i][1];
				...
				setGlobalState({...globalState, pred_date: results.data[0].slice(2).map(y=>y.concat('T23:00:00Z'))})
				setGlobalState({...globalState, case_pred_list: thislist})
				setGlobalState({...globalState, case_preds: results.data})
				doneLoading()
			}
		})
        ...
        for(i=1; i<results.data.length; i++)
        {
          if(results.data[i].length > 2){
            thislist[i-1] = results.data[i][1];
				...
				setGlobalState({...globalState, death_pred_list: thislist})
				setGlobalState({...globalState, death_preds: results.data})
				doneLoading()
			}
      })
	}, [])
  ...
  const updateWindowDimensions = () => {
		setGlobalState({...globalState, width: window.innerWidth, height: window.innerHeight})
	}

  const handleDataTypeSelect = e => {
		setGlobalState({...globalState, dataType: e})
	}
